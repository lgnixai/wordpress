...
<div id="page" class="site">
    <a class="skip-link screen-reader-text" href="#primary"><?php esc_html_e('Skip to content', 'export-theme'); ?></a>

    <!-- Header -->
    <header id="masthead" class="site-header sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <!-- Top Bar -->
        <div class="border-b">
            <div class="container">
                <div class="flex items-center justify-between py-2 text-sm">
                    <div class="flex items-center space-x-4">
                        <?php if ($phone = get_theme_mod('export_theme_company_phone')) : ?>
                            <a href="tel:<?php echo esc_attr($phone); ?>" class="flex items-center text-muted-foreground hover:text-primary transition-colors">
                        ...
                        <?php if ($email = get_theme_mod('export_theme_company_email')) : ?>
                            <a href="mailto:<?php echo esc_attr($email); ?>" class="flex items-center text-muted-foreground hover:text-primary transition-colors">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        foreach ($social_networks as $network => $icon) {
                            ...
                            if ($url) {
                                echo '<a href="' . esc_url($url) . '" target="_blank" rel="noopener" class="text-muted-foreground hover:text-primary transition-colors">' . $icon . '</a>';
                            }
                        }
                        ?>
                    </div>
        ...
        <nav class="container">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden md:block">
                    <?php
                    $cta_text = get_theme_mod('export_theme_header_cta_text', 'Get Quote');
                    $cta_url = get_theme_mod('export_theme_header_cta_url', '/contact');
                    ?>
                ...
                <!-- Mobile menu button -->
                <div class="md:hidden">
                    <button type="button" class="mobile-menu-toggle inline-flex items-center justify-center p-2 rounded-md text-gray-700 hover:text-primary hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary">
        ...
        <div class="mobile-menu hidden md:hidden">
            <div class="px-2 pt-2 pb-3 space-y-1">
                ...
                ?>
                <div class="pt-4">
                    <a href="<?php echo esc_url($cta_url); ?>" class="btn btn-primary btn-md w-full">
                        <?php echo esc_html($cta_text); ?>
                    </a>
                </div>
            </div>
