jQuery(document).ready(function($) {
    $('.notice[data-notice="get_started"]').on('click', '.notice-dismiss', function() {
        ...
        $.ajax({
            type: 'POST',
            data: {
                action: 'woodworking_workshop_dismiss_notice',
            },
            url: ajaxurl,
...
(function($) {
    $(document).on('click', '#woodworking-workshop-import-button', function(e) {
        e.preventDefault();
        const redirectUrl = $(this).attr('href');
        // Send AJAX request to dismiss the notice
        $.post(ajaxurl, { action: 'woodworking_workshop_dismiss_notice' }, function() {
